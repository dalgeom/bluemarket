   ...
   <nav>
    ...
    <div id="search">
        <form id="main_search" method="post" action="../main/pro_search">
         ...
         <input type="text" name="searchinput" placeholder="어떤 상품을 찾으시나요 ?"><button type="submit" id="searchbtn"><img src="../resources/img/dot.png" width="25" height="25"></button>
      </form>
   </div>
   ...
   <section>
   		...
   		<div class="proleft">
			...
			<c:if test="${userid == mvo.userid}">
				<div id="proleftbtn">
					<input type="button" value="내 상점 관리" id="plb" onclick="location='../manage/managelist'">
				</div>
			</c:if>
		</div>
   		<div class="proright">
   			<div class="prorfirst">
				...
				<!-- 레벨표시 -->
				<c:if test="${mvo.levp/10>=1 && mvo.levp/10<20 }"><!-- 1이상 20미만 -->
					&nbsp;<div style="color:orange"> <fmt:parseNumber type="number" integerOnly="true" value="${mvo.levp/10}"/>레벨</div>
				</c:if>
				<c:if test="${mvo.levp/10>=20 && mvo.levp/10<40}"> <!-- 20이상 40미만 -->
					&nbsp;<div style="color:#4FC3F7;"> <fmt:parseNumber type="number" integerOnly="true" value="${mvo.levp/10}"/>레벨</div>
				</c:if>
				<c:if test="${mvo.levp/10>=40 && mvo.levp/10<60}"> <!-- 40이상 60미만 -->
					&nbsp;<div style="color:#9575CD;"> <fmt:parseNumber type="number" integerOnly="true" value="${mvo.levp/10}"/>레벨</div>
				</c:if>
				<c:if test="${mvo.levp/10>=60 && mvo.levp/10<80}"> <!-- 60이상 80미만 -->
					&nbsp;<div style="color:#1E88E5;"> <fmt:parseNumber type="number" integerOnly="true" value="${mvo.levp/10}"/>레벨</div>
				</c:if>
				<c:if test="${mvo.levp/10>=80 && mvo.levp/10<100}"> <!-- 80이상 100미만 -->
					&nbsp;<div style="color:#FFA000;"> <fmt:parseNumber type="number" integerOnly="true" value="${mvo.levp/10}"/>레벨</div>
				</c:if>
				<c:if test="${mvo.levp/10>=100}">				  <!-- 100이상 -->
